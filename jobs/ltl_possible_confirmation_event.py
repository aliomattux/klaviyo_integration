...
class KlaviyoIntegrator(models.TransientModel):
    ...
    def execute_klaviyo_ltl_possible_confirmation_events(self, job):
        ...
        try:
            _logger.info('Downloading LTL Possible Agreement Sales data from Netsuite')
            conn = netsuite_obj.connection(job.netsuite_instance)
            response = conn.saved(vals)

        except Exception as e:
            subject = 'Could not get LTL Possible Agreement Sales data from Netsuite'
            self.env['integrator.logger'].submit_event('Klaviyo', subject, str(e), False, 'admin')
            return False

        try:
            agreements = self.process_ltl_possible_agreement_data(response['data'])
        ...
        sent_agreements = self.send_possible_agreement_events(agreements)
    ...
    def process_ltl_possible_agreement_data(self, response_data):
        agreements = {}
        for each in response_data:
            record = each['columns']
            record_id = record['internalid']['internalid']
            agreements[record_id] = self.convert_possible_agreement(record)

        return agreements


    def send_possible_agreement_events(self, agreements):
        agreement_data = []
        max = 20
        count = 0
        error_max_count = 5
        ...
        for sale_id, agreement in agreements.items():
            event_already_sent = self.find_klaviyo_event(sale_id, 'ltlpossibleagreement')
            if event_already_sent:
                agreement_data.append({
                    'id': sale_id,
                    'type': 'salesorder',
                    'field': 'custbody_k_dd_date_ltl_agreement',
                    'value': datetime.strftime(datetime.now(), '%m/%d/%Y')
                })
...
#                continue
            ...
            try:
                ...
                self.store_klaviyo_event_sent(sale_id, 'ltlpossibleagreement')
                sleep(0.3)
...
#                subject = 'Could not map and or send email in klaviyo for Fulfillment: %s'%fulfillment_id
 #               self.env['integrator.logger'].submit_event('klaviyo', subject, str(e), False, 'admin')
    ...
    def convert_possible_agreement(self, record):
